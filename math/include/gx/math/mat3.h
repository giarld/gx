...
#ifndef GX_MATH_MAT3_H
#define GX_MATH_MAT3_H
...
public:
    ...
    constexpr explicit TMat33(row_major_init c) noexcept
        : TMat33(std::move(c.m))
    {
    }
     ...
     */
    friend constexpr TMat33 orthogonalize(const TMat33 &m) noexcept
    {
     ...
     */
    constexpr static TMat33 getTransformForNormals(const TMat33 &m) noexcept
    {
        return matrix::cof(m);
    }
     ...
     * vector.
     */
    constexpr static TQuaternion<T> packTangentFrame(
        const TMat33 &m, size_t storageSize = sizeof(int16_t)) noexcept;

    template<typename A>
    constexpr static TMat33 translation(const TVec3<A> &t) noexcept
    {
        TMat33 r;
        r[2] = t;
        return r;
    ...
    template<typename A>
    constexpr static TMat33 scaling(const TVec3<A> &s) noexcept
    {
        return TMat33{s};
    }

    template<typename A>
    constexpr static TMat33 scaling(A s) noexcept
    {
...
template<typename T>
template<typename U>
constexpr TMat33<T>::TMat33(const TQuaternion<U> &q) noexcept
    : m_value{}
{
...
template<>
constexpr float dot_tolerance<float>() noexcept
{
    return 0.999f;
}

template<>
constexpr double dot_tolerance<double>() noexcept
{
    return 0.9999;
}
...
TMat33<T> TMat33<T>::lookTo(const TVec3<A> &direction, const TVec3<B> &up) noexcept
{
    ...
    const auto x_axis = normalize(cross(z_axis, norm_up));
    const auto y_axis = cross(x_axis, z_axis);
...
{
template<typename T>
constexpr void swap(math::details::TMat33<T> &lhs,
                    math::details::TMat33<T> &rhs) noexcept
{
