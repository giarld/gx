...
#ifndef GX_MATH_HALF_H
#define GX_MATH_HALF_H
...
class fp
{
    ...
    constexpr static unsigned S_SHIFT = E + M;
    constexpr static unsigned E_SHIFT = M;
    constexpr static unsigned M_SHIFT = 0;
    constexpr static unsigned S_MASK = ((1u << S) - 1u) << S_SHIFT;
    constexpr static unsigned E_MASK = ((1u << E) - 1u) << E_SHIFT;
    constexpr static unsigned M_MASK = ((1u << M) - 1u) << M_SHIFT;

    struct fp32
    {
        explicit constexpr fp32(float f) noexcept
            : fp(f)
        {
        } // NOLINT

        explicit constexpr fp32(uint32_t b) noexcept
            : bits(b)
        {
        } // NOLINT
...
public:
    constexpr static fp fromf(float f) noexcept
    {
        fp out;
        if (S == 0 && f < 0.0f) {
            return out;
    ...
    constexpr static float tof(fp in) noexcept
    {
    ...
    constexpr static size_t getBitCount() noexcept
    {
        return S + E + M;
    }
    ...
    explicit constexpr fp(TYPE bits) noexcept
        : bits(bits)
    {
    }
    ...
    constexpr void setS(unsigned int s) noexcept
    {
        bits = TYPE((bits & ~S_MASK) | (s << S_SHIFT));
    }

    constexpr void setE(unsigned int s) noexcept
    {
        bits = TYPE((bits & ~E_MASK) | (s << E_SHIFT));
    }

    constexpr void setM(unsigned int s) noexcept
    {
        bits = TYPE((bits & ~M_MASK) | (s << M_SHIFT));
    }

    constexpr unsigned int getS() const noexcept
    {
        return (bits & S_MASK) >> S_SHIFT;
    }

    constexpr unsigned int getE() const noexcept
    {
        return (bits & E_MASK) >> E_SHIFT;
    }

    constexpr unsigned int getM() const noexcept
    {
        return (bits & M_MASK) >> M_SHIFT;
    }
};
...
constexpr  uint16_t getBits(const half & h) noexcept {
    return MAKE_CONSTEXPR(reinterpret_cast<const uint16_t &>(h));
}

constexpr half makeHalf(uint16_t bits) noexcept {
    return MAKE_CONSTEXPR(reinterpret_cast<const half&>(bits));
}
...
public:
    ...
    constexpr half(float v) noexcept
        : mBits(fp16::fromf(v))
    {
    } // NOLINT
    constexpr operator float() const noexcept
    {
        return fp16::tof(mBits);
    } // NOLINT

private:
    // these are friends, not members (and they're not "private")
    friend constexpr uint16_t getBits(const half &h) noexcept
    {
        return h.mBits.bits;
    }

    friend constexpr half makeHalf(uint16_t bits) noexcept;
    ...
    explicit constexpr half(Binary, uint16_t bits) noexcept
        : mBits(bits)
    {
    }
...
public:
    ...
    constexpr static float_round_style round_style = round_indeterminate;
