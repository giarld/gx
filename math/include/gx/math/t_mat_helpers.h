...
#ifndef GX_MATH_TMATHELPERS_H
#define GX_MATH_TMATHELPERS_H
...
constexpr MATRIX gaussJordanInverse(MATRIX src)
{
    ...
    for (size_t i = 0; i < N; ++i) {
        ...
        for (size_t j = 0; j < N; ++j) {
            if (j != i) {
                const T value = src[j][i];
                for (size_t k = 0; k < N; ++k) {
                    src[j][k] -= src[i][k] * value;
                    inverted[j][k] -= inverted[i][k] * value;
                }
            }
        }
    }
...
public:
    ...
    constexpr static Matrix<T, ORDER - 1> submatrix(Matrix in, size_t row, size_t col) noexcept
    {
...
struct Determinant
{
    constexpr static T determinant(Matrix<T, O> in)
    {
        T det = {};
...
{
    constexpr static T determinant(Matrix<T, 3> in)
    {
        return
...
{
    constexpr static T determinant(Matrix<T, 2> in)
    {
...
{
    static constexpr T determinant(Matrix<T, 1> in)
    {
        return in[0][0];
    }
};
...
constexpr MATRIX cof(const MATRIX &matrix)
{
    return (MATRIX::NUM_ROWS == 2) ? fastCofactor2<MATRIX>(matrix) : ((MATRIX::NUM_ROWS == 3) ? fastCofactor3<MATRIX>(matrix) : cofactor<MATRIX>(matrix));
}
...
template<typename MATRIX,
    typename = std::enable_if_t<MATRIX::NUM_ROWS == MATRIX::NUM_COLS, int> >
constexpr typename MATRIX::value_type det(const MATRIX &matrix)
{
    typedef typename MATRIX::value_type T;
    constexpr unsigned int N = MATRIX::NUM_ROWS;
    Matrix<T, N> in{};
...
template<typename MATRIX,
    typename = std::enable_if_t<MATRIX::NUM_ROWS == MATRIX::NUM_COLS, int> >
constexpr MATRIX inverse(const MATRIX &matrix)
{
    return (MATRIX::NUM_ROWS == 2) ? fastInverse2<MATRIX>(matrix) : ((MATRIX::NUM_ROWS == 3) ? fastInverse3<MATRIX>(matrix) : gaussJordanInverse<MATRIX>(matrix));
}
...
template<typename MATRIX,
    typename = std::enable_if_t<MATRIX::NUM_ROWS == MATRIX::NUM_COLS, int> >
constexpr MATRIX transpose(MATRIX m)
{
...
template<typename MATRIX,
    typename = std::enable_if_t<MATRIX::NUM_ROWS == MATRIX::NUM_COLS, int> >
constexpr typename MATRIX::value_type trace(MATRIX m)
{
...
template<typename MATRIX,
    typename = std::enable_if_t<MATRIX::NUM_ROWS == MATRIX::NUM_COLS, int> >
constexpr typename MATRIX::col_type diag(MATRIX m)
{
...
private:
    ...
    //  matrix * matrix
    template<typename U>
    friend constexpr BASE<arithmetic_result_t<T, U> >
    ...
    // matrix * vector
    template<typename U>
    friend constexpr typename BASE<arithmetic_result_t<T, U> >::col_type
    ...
    // row-vector * matrix
    template<typename U>
    friend constexpr typename BASE<arithmetic_result_t<T, U> >::row_type
    operator*(const VEC<U> &lhs, const BASE<T> &rhs)
    {
        typename BASE<arithmetic_result_t<T, U> >
                ::row_type result{};
        for (size_t col = 0; col < BASE<T>::NUM_COLS; ++col) {
...
private:
     ...
     * (the first one, BASE<T> being known).
     */
    friend constexpr BASE<T> inverse(const BASE<T> &matrix)
    {
        return matrix::inverse(matrix);
    }

    friend constexpr BASE<T> cof(const BASE<T> &matrix)
    {
        return matrix::cof(matrix);
    }

    friend constexpr BASE<T> transpose(BASE<T> m)
    {
        return matrix::transpose(m);
    }

    friend constexpr T trace(BASE<T> m)
    {
        return matrix::trace(m);
    }

    friend constexpr T det(const BASE<T> &m)
    {
        return matrix::det(m);
    }

    // unclear why we have to use 'auto' here. 'typename BASE<T>::col_type' produces
    // error: no type named 'col_type' in 'math::details::TMat44<float>'
    friend constexpr auto diag(const BASE<T> &m)
    {
...
{
public:
    constexpr size_t getColumnSize() const
    {
        return BASE<T>::COL_SIZE;
    }

    constexpr size_t getRowSize() const
    {
        return BASE<T>::ROW_SIZE;
    }

    constexpr size_t getColumnCount() const
    {
        return BASE<T>::NUM_COLS;
    }

    constexpr size_t getRowCount() const
    {
    ...
    // array access
    constexpr const T *asArray() const
    {
        return &static_cast<const BASE<T> &>(*this)[0][0];
    }

    // element access
    constexpr const T &operator()(size_t row, size_t col) const
    {
        return static_cast<const BASE<T> &>(*this)[col][row];
    }
...
private:
    constexpr friend BASE<T> abs(BASE<T> m)
    {
        for (size_t col = 0; col < BASE<T>::NUM_COLS; ++col) {
            m[col] = abs(m[col]);
        }
